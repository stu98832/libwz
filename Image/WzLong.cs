...
namespace libwz
{
    ...
    public class WzLong : WzVariant
    {
        ...
        public override T GetValue<T>(T def = default(T))
        {
            ...
            {
                return (T)(object)(int)(this.Value & 0xFFFFFFFF);
            }
            else if (typeof(T) == typeof(uint))
            {
                return (T)(object)(uint)(this.Value & 0xFFFFFFFF);
            }
        ...
        {
            return this.Value == (obj as WzLong).Value;
        }

        /// <summary> </summary>
        public override WzVariant Clone()
        {
            return new WzLong(this.Name, this.Value);
        }

        internal override void Read(WzFileStream fs)
        {
            this.Value = fs.Read8(true);
        }

        internal override void Write(WzFileStream fs)
        {
            fs.Write8(this.Value, true);
        }
    }
}
